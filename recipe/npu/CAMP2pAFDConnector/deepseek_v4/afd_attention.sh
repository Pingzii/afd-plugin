...
ADDITIONAL_CONFIG="$(printf \
  '{"afd":{"role":"attention","connector":"CAMP2pAFDConnector","host":"%s","port":%s,"num_attention_ranks":%s,"num_ffn_ranks":%s}}' \
...
exec env VLLM_USE_V1=1 VLLM_USE_V2_MODEL_RUNNER=0 vllm serve "$MODEL_PATH" \
  ...
  --trust-remote-code \
  --served-model-name deepseek_v4_afd \
